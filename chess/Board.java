...
public class Board {
	...
	public static boolean movePiece(int i0, int j0, int i1, int j1, Piece p){
		if(board[i1][j1].hasPiece())
		{
			...
			if(getPiece(i1,j1).getSymbol().equals("♔") || getPiece(i1,j1).getSymbol().equals("♚"))//if the piece at destination is the king then return true
			{
			...
			board[i1][j1].removePiece();//else, if it has piece and its not the king replace it and return false
			board[i1][j1].setPiece(p);
			board[i0][j0].removePiece();
			return false;
		}
		...
		{
			board[i1][j1].setPiece(p);// if there is no piece at the destination then set the piece and return false
			board[i0][j0].removePiece();
	...
	//This method requires your input
	public static boolean hasPiece(int iIn, int jIn){
		if(getPiece(iIn, jIn)!=null)
			return true;
		else
			return false;
	}
